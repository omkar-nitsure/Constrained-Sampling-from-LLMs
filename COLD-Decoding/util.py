...
def beam_search_decode(model, y_logits, beam_width, x_onehot, x_past, tokenizer, batch_size, extra_mask=None):
    assert x_onehot.shape[1] == 1, x_onehot.shape
    length = y_logits.shape[1]
    past = x_past
    input_embeds = torch.matmul(x_onehot.float(), model.get_input_embeddings().weight)
    beams = [([], 0) for _ in range(beam_width)]  # Initialize beam with empty sequences
    for i in range(length):
        new_beams = []
        for beam in beams:
            seq, score = beam
            if len(seq) > 0:
                ...
                input_embeds = torch.matmul(one_hot(input_ids, tokenizer.vocab_size).float(), model.get_input_embeddings().weight)
            model_outputs = model(past_key_values=past, inputs_embeds=input_embeds)
            past = model_outputs.past_key_values
            logits_t = model_outputs.logits[:, -1:, :]
...
def get_text_from_sequence(sequence, tokenizer, batch_size):
    ...
    output_so_far = sequence.repeat(batch_size, 1)
