...
# def top_k_filter_3d_v2(logits, k, probs=False, mask=None, extra_mask=None):
#     """
...
def direct_decode(y_logits, tokenizer, batch_size):
    best_seq = torch.argmax(y_logits, axis=-1)

    return get_text_from_sequence(best_seq, tokenizer, batch_size)

def beam_search_varient(model, y_logits, beam_width, x_onehot, x_past, tokenizer, batch_size, topk_v2, extra_mask=None):
    assert x_onehot.shape[1] == 1, x_onehot.shape
    length = y_logits.shape[1]
    # print(y_logits.shape)
    # input_embeds = torch.matmul(x_onehot.float(), model.get_input_embeddings().weight)
    beams = [[([], 0) for _ in range(beam_width)] for __ in range(batch_size)]  # Initialize beam with empty sequences
    for b in range(batch_size):
        past = x_past
        input_embeds = torch.matmul(x_onehot[b].float(), model.get_input_embeddings().weight)
        mask_t_all = None
        for i in range(length):
            new_beams = []
            for beam in beams[b]:
                seq, score = beam
                # print(beams, "beams")
                # print(beams[b], "beams_b")
                # if len(seq) > 0:
                #     input_ids = torch.tensor(seq, dtype=torch.int64).unsqueeze(0).to(model.device)
                #     # input_ids = torch.tensor(seq, dtype=torch.int64).unsqueeze(0).repeat(batch_size,1).to(model.device)
                #     # input_embeds = torch.matmul(F.softmax(y_logits[b], dim=-1), model.get_input_embeddings().weight)
                #     input_embeds = torch.matmul(one_hot(input_ids, tokenizer.vocab_size).float(), model.get_input_embeddings().weight)[0]

                # if(past != None):
                #     print(past.shape, "model_outputs")
                # print(past, "past")
                # print(input_embeds.shape,"input_embeds")
                model_outputs = model(past_key_values=past, inputs_embeds=input_embeds)
                past = model_outputs.past_key_values
                logits_t = model_outputs.logits[-1:, :].unsqueeze(0)
                # print("logits_t", logits_t.shape)
                _, logits_top_indices = torch.topk(logits_t, topk_v2)
                mask_t = torch.zeros_like(logits_t).scatter_(2, logits_top_indices, 1)
                mask_t_all = mask_t if mask_t_all is None else torch.cat((mask_t_all, mask_t), dim=1)
                log_probs = F.log_softmax(logits_t, dim=-1).squeeze(1)
                top_log_probs, top_indices = torch.topk(log_probs, topk_v2)

                # print(top_log_probs.shape, "top_log_probs")
                # print(top_indices.shape, "top_indices")
        ...
        # logits_so_far = logits_t if logits_so_far is None else torch.cat((logits_so_far, logits_t), dim=1)
                if i < length - 1:
                    if extra_mask is None:
                        y_logits_i_topk = top_k_filter_3d(y_logits[b,i:i+1,:].unsqueeze(0), beam_width, mask=mask_t) / 0.001
                    else:
                        y_logits_i_topk = top_k_filter_3d(y_logits[b,i:i+1,:].unsqueeze(0), beam_width, mask=mask_t, extra_mask=extra_mask[:,i:i+1,:]) / 0.001
                    input_embeds = torch.matmul(F.softmax(y_logits_i_topk, dim=-1), model.get_input_embeddings().weight)[0]
                    # Select top beams based on scores
            new_beams.sort(key=lambda x: x[1], reverse=True)
            beams[b] = new_beams[:beam_width]
        # Return the sequence with the highest score
    # # print(len(beams))
    # # print(len(beams[0]))
    # # print(len(beams[0][0]))
    # print(beams[0], "beams1")
    # print(beams[0][0], "beams2")
    best_seq = []
    for b in range(x_onehot.shape[0]):
        best_seq.append(beams[b][0][0])
...
def get_text_from_sequence(sequence, tokenizer, batch_size):


    output_so_far = torch.tensor(sequence, dtype=torch.int64)
...
# def decode_with_model_beam_search_k(model, y_logits, k, x_onehot, x_past, tokenizer, extra_mask=None):
#     assert x_onehot.shape[1] == 1, x_onehot.shape
